 ...
 * 4. Construct request to treat vdx paper
...
async function main() {
    ...
    try {
        ...
        const treatResponse = await contract.submitTransaction('treat', 'PC', '00001', 'RQ', 'PC', '2022-11-21', "", "5000");

        // process response
        console.log('Process treat transaction response.'+treatResponse);

        let paper = VdxPaper.fromBuffer(treatResponse);

        console.log(`${paper.issuer} vdx paper : ${paper.paperNumber} successfully treat for user ${paper.fullname}`);
